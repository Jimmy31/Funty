...
| Mathématiques | Dénombrement | Un nombre d'objets d'un même type (ex. des chats, des tractopelles — un type d'objet différent à chaque itération) est affiché à l'écran, disposé aléatoirement (position, orientation, taille) sans chevauchement ; l'enfant compte puis dit le total à voix haute ou le choisit parmi les propositions. **Variante 1 à 5 objets.** | Vocal + Tactile | PS |
| Mathématiques | Dénombrement | Même exercice que ci-dessus. **Variante 1 à 10 objets**, pour aller plus loin une fois la variante 1 à 5 maîtrisée. | Vocal + Tactile | PS |
...
Pour les deux additions, l'enfant pourra donc soit dire le résultat à voix haute, soit le saisir au tactile (QCM, glisser-déposer ou saisie numérique) — l'app acceptant l'une ou l'autre réponse indifféremment, via la bascule décrite en 6.2.
...
- **Disposition aléatoire sans chevauchement — implémentée (2026-07-29)** : position, orientation et taille aléatoires pour chaque objet, avec une contrainte de non-chevauchement garantie par un algorithme de placement (nouvelle tentative si une position choisie chevauche un objet déjà placé). Paramètres retenus : rotation limitée à **±18°** (au-delà, l'animal devient pénible à identifier pour un enfant de PS), variation de taille de **±18 %** autour d'une taille de base qui **décroît avec le nombre d'objets**, pour garder la même densité visuelle qu'il y en ait 1 ou 10. Quand le cadre est trop encombré pour loger un objet à sa taille tirée, il est **rétréci puis replacé** plutôt que posé sur un voisin — la contrainte de non-chevauchement est donc tenue dans tous les cas. Le tirage est fait **une fois par question** (en coordonnées normalisées, indépendantes de la taille de l'écran) : le recalculer à chaque reconstruction ferait sauter les objets d'un endroit à l'autre sous les yeux de l'enfant, même piège que les propositions QCM (cf. 6.2). Couvert par des tests automatisés (`test/object_scatter_test.dart`), le risque identifié en section 10 étant précisément la fiabilité et le coût de ce placement.
- **Saisie tactile ajoutée (2026-07-29)** : les deux variantes passent de "vocal" à **"vocal + tactile"** et proposent donc la bascule décrite en 6.2 — l'enfant peut dire le total ou le choisir parmi les propositions. Le plafond des propositions suit la variante (5 ou 10).
...
- **Propositions QCM confirmées : 8 réponses fixes, affichées sur 2 lignes de 4, groupées autour de la bonne valeur, jamais au-delà du plafond de l'exercice (revirement — 8 fixes plutôt que 5 à 8 aléatoire, pour un affichage compact et prévisible, cf. 8.1 2026-07-26).** Pour les exercices Addition (≤5/≤10/≤20), aucune proposition ne doit dépasser le plafond de l'exercice (ex. jamais de "6" proposé sur Addition ≤5) — les valeurs voisines de la bonne réponse sont piochées des deux côtés jusqu'à réunir 8 propositions, en respectant cette borne (et 0 en borne basse). Peut donner moins de 8 propositions (sur une seule ligne partielle) si peu de valeurs valides existent à proximité (ex. bonne réponse = plafond de l'exercice). **Tirées une seule fois par question et fixes ensuite** : elles ne doivent pas changer tant que la question reste affichée, même quand l'écoute vocale continue déclenche des mises à jour d'écran fréquentes (cf. bug corrigé ci-dessous).
...
- **Consigne orale systématique** : chaque exercice, quel que soit son mode de réponse, explique sa tâche à l'enfant **à l'oral** (ex. "Compte les chats et dis le nombre à voix haute"), et pas seulement par texte ou pictogramme — indispensable pour les enfants qui ne savent pas encore lire (PS notamment). C'est une métadonnée/asset obligatoire de chaque exercice, pas une option. **Placeholder texte retiré de l'écran d'exercice (2026-07-26, cf. 8.1)** : en attendant le véritable enregistrement audio, le texte de la consigne prenait trop de place à l'écran (l'objectif visuel étant de tout afficher sans avoir à faire défiler l'écran) ; il n'est donc plus affiché, sans que cela ne change l'exigence de fond ci-dessus — la vraie consigne orale (audio) reste à implémenter.
...
- **Écoute vocale automatique, sans bouton à presser.** Dès qu'une question apparaît (exercice vocal ou vocal + tactile en mode vocal), la reconnaissance démarre toute seule et reste active en continu — l'enfant répond directement à voix haute, sans geste préalable. Un simple indicateur passif (icône micro) signale l'état de l'écoute ; ce n'est pas un bouton. **Texte "Je t'écoute..." retiré (2026-07-26, cf. 8.1)** : redondant avec le changement de couleur de l'icône, et supprimé pour gagner de la hauteur d'écran (même objectif que le retrait du placeholder de consigne orale ci-dessus). **Texte de débogage réintroduit à côté du micro (2026-07-26, cf. 8.1)** : distinct du texte "Je t'écoute..." retiré ci-dessus — affiche le mot reconnu en temps réel (résultat partiel, puis final) à des fins de diagnostic pendant les tests, pas un élément destiné à l'enfant à terme.
- **Indicateur micro tricolore (2026-07-28, cf. 8.1)** : l'icône du micro distingue trois situations, pour que l'enfant (et le parent qui l'accompagne) comprenne *pourquoi* rien ne se passe — **gris** = rien d'audible capté, **rouge** = du son capté mais aucun mot attendu reconnu (`[unk]` côté Vosk), **vert** = un mot attendu reconnu. Un `[unk]` **ne compte pas comme une réponse fausse** : ce n'est pas une tentative de l'enfant mais une captation ratée, il ne déclenche donc ni décompte d'échec, ni recul de la grenouille (cf. 6.7bis), ni pénalité de temps (cf. 6.5).
- **Bascule vocal / tactile sur les exercices qui acceptent les deux (2026-07-28, cf. 8.1)** : au lieu de laisser micro et boutons actifs simultanément, l'écran d'exercice porte un sélecteur à deux positions explicites — **"Je dis"** (icône micro) et **"Je touche"** (icône main). Une seule entrée est affichée et active à la fois ; passer en mode tactile **arrête réellement l'écoute** plutôt que de la laisser tourner en fond. Le mode vocal reste celui par défaut à l'ouverture d'un exercice. Ceci précise le principe posé plus haut ("l'enfant choisit librement comment répondre") en lui donnant une commande visible, au lieu d'un choix implicite.
- **Grammaire de reconnaissance à l'échelle de l'exercice, pas de la question (2026-07-28, cf. 8.1)** : la grammaire fermée soumise à Vosk couvre **toutes les réponses attendues de l'exercice** (les 26 lettres, les nombres de la plage, les noms de formes…), posée une seule fois à l'ouverture. Deux raisons : (1) l'app peut transcrire *ce que l'enfant a réellement dit* quand il se trompe, au lieu d'un `[unk]` indistinct ; (2) la grammaire n'est plus reconfigurée en cours d'écoute, ce qui était la cause des plantages (cf. 8.1). La reconnaissance reste bien limitée à un **mot isolé** par réponse (principe inchangé, cf. 5.1).
...
### 6.7bis Animation de progression pendant la série (exercices de calcul)
- **Périmètre : les 4 exercices de calcul uniquement** — Addition ≤5, ≤10, ≤20 et Soustraction ≤10. Les autres exercices du MVP gardent leur affichage habituel ; cette animation ne remplace ni le flash bonne/mauvaise réponse (cf. 6.2), ni le badge de fin de série (cf. 6.7) — elle s'y ajoute comme retour visuel *pendant* la série, question après question, là où le badge ne se voit qu'à la fin.
- **Principe : une grenouille progresse le long d'un toboggan de piscine**, illustrée par une séquence de **12 images** (banque `assets/images/grenouille/`, extraite de la planche source `docs/graphics/animation grenouille.png` — fond magenta `#FF00FF` détouré en transparence). Position de départ : image 1 (bas de l'échelle, tout début de série). Position finale : image 12 (la grenouille fête son arrivée dans la piscine).
- **Règle d'évolution, par tentative de réponse** :
  - Bonne réponse donnée en **5 secondes ou moins** → avance d'une image.
  - Mauvaise réponse, **ou** bonne réponse mais prise en **plus de 5 secondes** → recule d'une image.
  - Le chronomètre de cette règle se mesure **par tentative** (pas depuis le début de la question) : il redémarre après chaque tentative, y compris après un premier échec sur la même question — cohérent avec le fait que l'enfant réessaie alors la même question.
  - La position reste toujours comprise entre l'image 1 et l'image 12 (pas de recul en dessous du départ, pas d'avance au-delà de l'arrivée).
- **Portée : uniquement visuelle, pas de score propre.** Arriver à l'image 12 ne déclenche aucun message ni badge particulier — l'animation est un retour d'engagement immédiat, distinct du système de badges bronze/argent/or (temps moyen par question sur la série, cf. 6.7) qui reste la mesure de référence de la maîtrise de l'enfant.
- **Réinitialisée à chaque série** : la position repart de l'image 1 à chaque nouvelle série de l'exercice (pas de persistance d'une séance à l'autre) — cohérent avec le fait qu'une série est un passage complet et autonome (cf. 6.2, indicateur de progression "x / y").
...
- **Vocabulaire vocal 11-20 non encore validé sur appareil** : contrairement au vocabulaire 0-10 et aux lettres (validés via le parcours systématique du spike, cf. section 12), les mots "onze" à "vingt" — notamment les nombres composés "dix-sept"/"dix-huit"/"dix-neuf" — n'ont pas été soumis au même test de reconnaissance réelle. Point à vérifier en priorité si des échecs de reconnaissance apparaissent sur l'exercice Addition ≤ 20, sur le même principe que la résolution des lettres H/M/N/X en son temps. **Premier signal en test réel (2026-07-26)** : suspicion de non-reconnaissance de "vingt" (20) sur l'exercice Addition ≤ 20 — à confirmer via le texte de débogage affiché à côté du micro (cf. 6.2) avant de conclure à un problème de vocabulaire comparable au cas du Z (cf. section 12).
...
### Décisions actées (2026-07-26)

- **Animation de progression "grenouille au toboggan" ajoutée sur les 4 exercices de calcul** (Addition ≤5/≤10/≤20, Soustraction ≤10) — cf. 6.7bis pour le détail complet. Séquence de 12 images, avance d'une image sur bonne réponse ≤ 5s, recule sur erreur ou réponse > 5s, réinitialisée à chaque série. Distincte du badge de fin de série (6.7), qui reste inchangé.
- **Banque d'illustrations acquise pour cette animation** : planche source `docs/graphics/animation grenouille.png` (12 vignettes sur fond magenta `#FF00FF`), détourée en 12 images individuelles transparentes dans `assets/images/grenouille/`.
- **Dénombrement illustré avec de vraies images d'animaux (revirement partiel sur le contenu, pas sur la mécanique)** : les deux exercices Dénombrement affichent désormais des illustrations réelles plutôt qu'un texte générique ("3 chats"). **7 familles d'animaux disponibles** (coccinelles, chats, chiens, poissons, oiseaux, insectes, singes), 10 images distinctes par famille, détourées depuis des planches sources dans `docs/graphics/`. **La famille change à chaque question** (jamais deux fois de suite la même) et les images affichées sont tirées aléatoirement dans la famille choisie — cohérent avec la "banque d'illustrations par catégorie" déjà prévue en 5.1/10, désormais concrétisée pour ces 7 familles.
- **Écran d'exercice compacté suite aux premiers essais réels sur appareil** : trois retraits pour que l'écran tienne sans défilement — (1) le texte "Je t'écoute..." disparaît, seule la couleur de l'icône micro indique l'écoute active (cf. 6.2) ; (2) le texte de consigne orale disparaît de l'écran (placeholder en attendant la vraie consigne audio, cf. 6.2) ; (3) les propositions QCM passent de "5 à 8 aléatoire" à **8 fixes, affichées sur 2 lignes de 4** (cf. 6.2). Éléments visuels (grenouille, images de dénombrement) et espacements également resserrés. Aucun changement de mécanique de réponse, uniquement de présentation.
- **Bug corrigé : débordement horizontal (overflow) des 8 boutons QCM sur 2 lignes de 4**, apparu sur l'exercice Addition ≤ 20 avec des propositions à 2 chiffres (ex. "20") plus larges que prévu. Chaque bouton occupe désormais une largeur égale (1/4 de ligne) via un layout en `Expanded`, garantissant qu'il tient toujours dans l'écran quel que soit le nombre de chiffres affichés — cf. 6.2.
- **Texte de débogage de la reconnaissance vocale réintroduit à côté du micro**, retiré par erreur lors du compactage de l'écran ci-dessus (il avait été confondu avec le texte "Je t'écoute...", pourtant distinct et utile pour diagnostiquer les échecs de reconnaissance pendant les tests) — cf. 6.2. Remonté suite à une suspicion de non-reconnaissance de "vingt" (20) sur l'exercice Addition ≤ 20, à confirmer avec ce texte réactivé (cf. section 10, vocabulaire 11-20 non encore validé).
- **Bug corrigé : la grammaire vocale des exercices vocal + tactile (Addition ×3, Soustraction) ne contenait que le mot de la bonne réponse.** Le texte de débogage ci-dessus a permis de diagnostiquer la cause réelle : "vingt" était bien reconnu **quand c'était la bonne réponse**, mais tout autre nombre prononcé par l'enfant (une mauvaise réponse) ne pouvait être transcrit qu'en "[unk]" faute de figurer dans la grammaire — alors que la reconnaissance doit couvrir **tous les nombres valides de la plage de l'exercice** (0 à 20 pour Addition ≤20, etc.) et les traiter comme une erreur s'ils ne correspondent pas à la bonne réponse, pas seulement détecter "ce n'est pas le mot attendu". La grammaire de ces exercices couvre désormais tous les mots-chiffres de 0 à la borne de l'exercice (`maxAnswerValue`, ou 20 par défaut pour Soustraction qui n'a pas de borne définie) ; les exercices purement vocaux à réponse unique (Alphabet, Nombres, Formes, Comptage, Dénombrement) gardent leur grammaire resserrée au seul mot attendu, non concernés par ce bug (une seule réponse valide possible, pas une plage).

### Décisions actées (2026-07-28)

- **Bug corrigé : l'exercice Addition ≤ 20 ne proposait que des résultats compris entre 13 et 20.** Les quatre exercices de calcul reposaient sur une liste de 5 paires écrites en dur, choisies au moment de l'amorçage du catalogue et jamais revues — Addition ≤ 20 ne contenait par exemple que 8+5, 9+7, 12+6, 13+4 et 11+9. Les questions sont désormais **générées depuis la borne de l'exercice** et couvrent tous les résultats de la plage (cf. 5.1) : Addition ≤ 5 passe de 5 à 8 questions, ≤ 10 de 5 à 23, ≤ 20 de 5 à 53, Soustraction ≤ 10 de 5 à 25. Le même défaut touchait les trois autres exercices de calcul (Soustraction ≤ 10 ne proposait que les résultats 4, 5 et 6) et a été corrigé en même temps.
- **`maxAnswerValue` renseigné sur la Soustraction ≤ 10**, qui n'en avait pas : sans borne, les propositions QCM pouvaient dépasser 10 et la grammaire vocale s'étendait inutilement jusqu'à 20 (cf. 6.2).
- **Bug corrigé : plantages réguliers de l'application, cause native identifiée.** Les traces système (`dumpsys dropbox`) montrent des `abort()` sur assertion Kaldi dans `AcceptWaveform`, levés depuis le thread de reconnaissance de `libvosk.so`. Cause : la grammaire était reconfigurée à chaque nouvelle question, alors que l'arrêt de la capture (`stop()`) n'interrompt pas immédiatement le thread natif — celui-ci pouvait donc être en train d'alimenter le recognizer pendant sa reconfiguration. Corrigé en posant **une grammaire unique pour tout l'exercice, avant la première écoute** (cf. 6.2), si bien qu'aucune reconfiguration n'a plus lieu en cours de session.
- **Trois autres fuites/races corrigées dans la foulée, sur le même symptôme** : (1) les abonnements aux flux de résultats Vosk n'étaient jamais annulés et survivaient à l'écran d'exercice, alors que le plugin est un singleton réutilisé par l'exercice suivant ; (2) sortir d'un exercice pendant le chargement du modèle laissait un service d'écoute orphelin derrière lui ; (3) un résultat vocal arrivant en retard (le flux continue de livrer son tampon après `stop()`) pouvait être traité pendant la révélation ou la fin de série et enclencher une seconde transition de question en parallèle de la première.
- **Latence de capture vocale réduite** : la réponse n'attend plus que Vosk détecte la fin de parole (un silence assez long, ressenti comme une lenteur). Un résultat **partiel** est validé dès qu'il correspond à un mot attendu, sous deux garde-fous : le mot ne doit être le début d'aucun autre mot de la grammaire (« dix » attend, puisque « dix-sept » existe ; « vingt » est validé tout de suite), et il doit rester stable 400 ms — Vosk révisant ses partiels en cours de route (« trois » → « treize »), valider le premier venu enregistrerait une réponse fausse.
- **Comparaison des réponses vocales rendue insensible à la casse, aux traits d'union et aux espaces multiples** : selon le découpage du modèle, Vosk peut rendre « dix-sept » sous la forme « dix sept », ce qui aurait été compté comme une erreur.
- **Bascule vocal / tactile et indicateur micro tricolore ajoutés** — cf. 6.2 pour le détail des deux.

### Décisions actées (2026-07-29)

- **Disposition aléatoire du Dénombrement implémentée**, avec ses paramètres visuels arrêtés (rotation ±18°, taille ±18 % autour d'une base décroissante avec le nombre d'objets, rétrécissement plutôt que chevauchement quand le cadre est saturé) — cf. 5.1. Ceci solde le point 4 des prochaines étapes (section 11) et referme le risque "algorithme de placement non-chevauchant" de la section 10, désormais couvert par des tests automatisés.
- **Saisie tactile ajoutée aux deux exercices Dénombrement**, qui passent de "vocal" à "vocal + tactile" — cf. 5.1 et la bascule décrite en 6.2. Le catalogue compte donc désormais **6 exercices vocal + tactile** (Addition ×3, Soustraction, Dénombrement ×2) et 7 purement vocaux.
...
- ~~**Algorithme de placement non-chevauchant (Dénombrement)**~~ — **risque levé (2026-07-29)** : implémenté et couvert par des tests automatisés (non-chevauchement garanti de 1 à 10 objets, objets entièrement contenus dans le cadre, temps de calcul mesuré). La difficulté anticipée s'est confirmée sur un point précis : avec **peu** d'objets — donc de grands objets — la zone où leur centre peut tomber sans déborder du cadre devient si petite que deux voisins suffisent à la saturer, et le tirage échouait alors sur un léger chevauchement. Résolu en rétrécissant l'objet puis en le replaçant, plutôt qu'en acceptant le recouvrement.
